...
export default function SessionPlayer() {
  ...
  return <div className={`session-player${started ? " is-playing" : ""}`}>
    <video ref={video} controls={started} tabIndex={started ? 0 : -1} playsInline preload="none" aria-label="Marcelo's acoustic guitar session">
      <source src="/media/soul-session.mp4" type="video/mp4" />
      Your browser cannot play this video. <a href="/media/soul-session.mp4">Open the guitar session</a>.
    </video>
    {!started && <button type="button" className="session-cover" onClick={playSession} aria-label="Play acoustic guitar session with sound">
      ...
      <span className="session-play"><span className="round-play" aria-hidden="true">▶</span><span>Play the session<small>Acoustic / with sound</small></span><span className="session-duration">04:29</span></span>
    </button>}
  </div>;
}
